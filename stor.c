...
int readFile1(char* content1,char* key, char* tgatbuff, char* nonce) {
                        // No need to search for ':', as nonce is now stored in the user structure
    char* hexCiphertext = content1;

    // Convert hex string to binary
    unsigned char ciphertext[strlen(hexCiphertext) / 2];
    if (sodium_hex2bin(ciphertext, sizeof(ciphertext), hexCiphertext, strlen(hexCiphertext), NULL, NULL, NULL) < 0) {
        // Conversion error
        return 0;
    }
    size_t ciphertext_len = strlen(ciphertext) - crypto_secretbox_MACBYTES;
    // Decrypt the content
    ...
    //printf("the extracte old content is %s",tgatbuff);
    return 1;
}
...
int readFile(char* username, char* filename) {
    ...
    while (currentUser != NULL) {
        if (strcmp(currentUser->username, username) == 0) {
            ...
            while (currentFile != NULL) {
                if (strcmp(currentFile->filename, filename) == 0) {
                    // No need to split nonce and ciphertext, as nonce is stored in the user structure
